...
echo ""
echo "=== 1/15  OHLCV price data ==="
python ingestion/ohlcv_ingestion.py

echo ""
echo "=== 2/15  Short interest (FINRA) ==="
python ingestion/short_interest_ingestion.py

echo ""
echo "=== 3/15  Earnings surprises ==="
python ingestion/earnings_ingestion.py

echo ""
echo "=== 4/15  News articles (GDELT + RSS) ==="
python ingestion/news_ingestion.py

echo ""
echo "=== 5/15  NLP sentiment scoring (FinBERT) ==="
python processing/nlp_pipeline.py

echo ""
echo "=== 6/15  Sentiment features ==="
python processing/sentiment_features.py

echo ""
echo "=== 7/15  Graph features ==="
python processing/graph_features.py

echo ""
echo "=== 8/15  13F institutional holdings (incremental) ==="
python ingestion/sec_13f_ingestion.py

echo ""
echo "=== 9/15  Ownership features ==="
python processing/ownership_features.py

echo ""
echo "=== 10/15  SAM.gov government contract awards ==="
python ingestion/sam_gov_ingestion.py

echo ""
echo "=== 11/15  FERC interconnection queue ==="
python ingestion/ferc_queue_ingestion.py

echo ""
echo "=== 12/15  USPTO patent applications + grants ==="
python ingestion/uspto_ingestion.py

echo ""
echo "=== 13/15  USAJOBS federal AI/ML job postings ==="
python ingestion/usajobs_ingestion.py

echo ""
echo "=== 14/15  BLS JOLTS tech sector job openings ==="
